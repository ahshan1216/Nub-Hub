...
function checkStudentStatusAndRedirect($student_id, $short_name, $session, $redirect_url,$active)
{
    global $connection;
    ...
    if ($team_result->num_rows > 0) {
        ...
        die(json_encode(['success' => false, 'message' => 'Student not part of any team']));
    }

    // Fetch project_open from teacher_slot table
...
// Assuming the required variables are already fetched
...
if ($esult3->num_rows > 0) {
    ...
    $database_file_size = $ow3['storage'];
    $_SESSION['size1']=$database_file_size;
    $session = $ow3['session'];
    $short_name = $ow3['short_name'];
    $active = $ow3['active'];
}
$redirect_url = 'restricted.php'; // Replace with your redirect URL

$result = checkStudentStatusAndRedirect($student_id, $short_name, $session, $redirect_url,$active);

if ($result['success']) {
    // echo "Leader ID: " . $result['leader_id'];
...
$database_file_size = bytesToMegabytes($database_file_size);
/**
 * H3K | G File Mananger V2.5.3
 * @author Ahshan Habib
 * @github https://grohonn.com
 * @link grohonn.com
 */
//$ami_files_size=0;
...
// Generate secure password hash - grohonn.com
$auth_users = array(
...
if (isset($_SESSION[FM_SESSION_ID]['logged'])) {
    ...
    if (md5($password) === $db_password && verifyToken($_POST['token'])) {
...
$root_path = $_SERVER['DOCUMENT_ROOT'] . '/project/' . 'project_file/' . $site_url1.$session;
// echo "<script>console.log('Root Path: " . $_SESSION['site_url'] . "');</script>";
echo "<script>console.log('Root Path: " . $root_path . "');</script>";
$_SESSION['full_site_ur']=$site_url1.$session;
    ...
    <div class="path">
        ...
        <div class="card mb-2 fm-upload-wrapper <?php echo fm_get_theme(); ?>">
            ...
            <div class="card-body">
                ...
                if ($database_file_size > $ami_files_size) {

                    $session_token = $_SESSION['token'];
                    $sub = mbToBytes($database_file_size) - $_SESSION['size'];
                    echo 'Your Remaining Upload Size is  ' . roundToTwoDecimals(bytesToMegabytes($sub)) . 'MB';

                } else {
                    $session_token = $_SESSION['token1'];
                    echo $sub;
                    $sub = mbToBytes($database_file_size) - $_SESSION['size'];

                }
                $max_upload_size = bytesToMegabytes($sub);
                ?>
        ...
        Dropzone.options.fileUploader = {
            ...
            init: function () {
                this.on("sending", function (file, xhr, formData) {
                    ...
                    console.log(res.xhr.response); // Inspect the raw response
                    try {
                        let _response = JSON.parse(res.xhr.response);
                        console.log(_response); // Check the parsed JSON object

                        if (_response.status == "error") {
                            toast(_response.info);
                        }
                    } catch (e) {
                        toast("Error: Invalid JSON1 response");
                    }
            ...
            <div class="card-body">
                <div class="row">
                    <div class="col-xs-12 col-sm-6">
                        <p>
                        <h3><a href="https://grohonn.com" target="_blank" class="app-v-title"> G File Mananger
                                <?php echo VERSION; ?></a></h3>
                        </p>
                        <p>Author: Ahshan Habib</p>
                        <p>Mail Us: <a href="mailto:grohonshop@gmail.com">grohonshop[at]gmail.com</a> </p>
                    </div>
                    <div class="col-xs-12 col-sm-6">
                        <div class="card">
                            <ul class="list-group list-group-flush">
                                <li class="list-group-item"><a href="https://grohonn.com" target="_blank"><i
                                            class="fa fa-question-circle"></i> <?php echo lng('Help Documents') ?> </a>
                                </li>
                                <li class="list-group-item"><a href="https://grohonn.com" target="_blank"><i
                ...
                <?php if ($is_text) { ?>
                    <?php if ($isNormalEditor) { ?>
                        ...
                        <button type="button" class="btn btn-sm btn-success" name="Save"
                            data-url="<?php $parsed_url = parse_url($file_url, PHP_URL_PATH);
                            $full_path2 = $root_path . $parsed_url;
                            echo fm_enc($full_path2); ?>"
...
<form action="" method="post" class="pt-3">
    ...
    <style>
        ...
        .database-btn:hover {
            ...
            /* Border matching the hover background color */
        }
    </style>
    <div class="row">
        <?php if (!FM_READONLY): ?>
            <div class="col-xs-12 col-sm-9">
                <ul class="list-inline footer-action">
                    ...
                    <li class="list-inline-item">
                        <a href="databasecredential.php"
                            class="btn btn-small btn-outline-primary btn-2 database-btn">
                            <i class="fa fa-database"></i> Click here For Credential
                        </a>
                    </li>

                </ul>
            </div>
            <div class="col-3 d-none d-sm-block"><a href="grohonn.com" target="_blank" class="float-right text-muted">G File
                    Mananger <?php echo VERSION; ?></a></div>
        <?php else: ?>
            <div class="col-12"><a href="grohonn.com" target="_blank" class="float-right text-muted">G File Mananger
...
class FM_Config
{
    ...
    function __construct()
    {
        ...
        if (strlen($CONFIG)) {
            ...
            $msg = 'G File Mananger<br>Error: Cannot load configuration';
            if (substr($fm_url, -1) == '/') {
                $fm_url = rtrim($fm_url, '/');
                $msg .= '<br>';
...
function fm_show_header_login()
{
    ...
    <head>
        ...
        <meta name="description" content=" G File Mananger">
        <meta name="author" content="Grohon">
