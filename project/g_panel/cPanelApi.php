...
public function addNewDomain($newDomain, $subdomain, $folder = '') {
    // Assign parameters
    $domain = $newDomain;
    $subdomain = $subdomain;
    ...
    if ($folder == '') {
        $folderdir = '/home/etisparl/public_html/' . $newDomain;
    } else {
        $folderdir = '/home/etisparl/public_html/' . $folder;
    }

    // URL encode the directory path
    $encodedFolderdir = urlencode($folderdir);
    
    // Construct the API query for addon domain
    $query = "https://$this->cPanelUrl:$this->cPanelPort/json-api/cpanel?cpanel_jsonapi_module=AddonDomain&cpanel_jsonapi_func=addaddondomain&subdomain=$subdomain&newdomain=$domain&dir=$encodedFolderdir";

    // Execute the cPanel API call
    return $this->exe_cpanel($query);
}
